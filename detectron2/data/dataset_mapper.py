...
class DatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        ######################################################
        image = to_nine_dim(image)
        #image = to_six_dim(image)
...
def ela(img, quality, error_scale, error_contrast):
    """
    ...
    Params:
        ...
        save_dir:   save path
    """
    tmp_fname = "tmp/" + str(os.getpid()) + str(random.randint(0, 50000000)) + ".jpg"
    ...
    scale = 255.0/max_diff * error_scale
    ela_im = ImageEnhance.Brightness(ela_im).enhance(scale)

    #change contrast
    contrast_enhancer = ImageEnhance.Contrast(ela_im)
    ela_im = contrast_enhancer.enhance(error_contrast)#.convert("L")
    #print(np.average(ela_im))

    os.remove(tmp_fname)
    #ela_im.save(tmp_fname, 'JPEG', quality = 100)
...
def normal_map(img, intensity=10, smooth=0):
    img = imageio.core.util.Array(img)
    if img.ndim == 3:
        img_grey = np.zeros((img.shape[0],img.shape[1])).astype(float)
        img_grey = (img[...,0] * 0.3 + img[...,1] * 0.6 + img[...,2] * 0.1)
        img = img_grey

    img_smooth = smooth_gaussian(img, smooth)

    sobel_x, sobel_y = sobel(img_smooth)

    normal_map = compute_normal_map(sobel_x, sobel_y, intensity)
    #normal_map = cv2.cvtColor(normal_map, cv2.COLOR_BGR2GRAY)

    return img_as_ubyte(normal_map)

def to_nine_dim(img):
    ela_img = ela(img, 90, 2, 3)
    nm_img = normal_map(img, 20, 0)

    nine_dim = np.zeros([img.shape[0], img.shape[1], 9]).astype(np.float32)
    nine_dim[:, :, :3] = img
    nine_dim[:, :, 3:6] = ela_img
    nine_dim[:, :, 6:] = nm_img
    return nine_dim.astype(np.float32)

def to_six_dim(img):
    nm_img = normal_map(img, 20, 0)

    six_dim = np.zeros([img.shape[0], img.shape[1], 6]).astype(np.float32)
    six_dim[:, :, :3] = img
    six_dim[:, :, 3:] = nm_img
    return six_dim.astype(np.float32)
